...
import asyncio
from host_selector import host_selector
...
async def main():
    ...
    #when using host selector, mode should be 0 algo should be 0
    portmap_server = portmap(config["port"], config["target_addr"], config["target_port"], base_encode.generate(config["algo"]), config.get("mode", 0), host_selector())
